...
\begin{afixme}
  No entiendo este environment thebibliography si ya tienes fichero bib.
\end{afixme}
...
\fixme{No es necesario que pongas cualquier diagrama o imagen dentro de una figura, puedes incrustarla entre los párrafos.}

\section{Arquitectura de la aplicación}	
%\fxnote{He promocionado esto a sección.}
   
   Como ya hemos visto en apartados anteriores, la aplicación esta formada principalmente por un conjunto de controladores de vista más un módulo encargado de comunicarse con la interfaz de los servicios web de Betfair. A este módulo le llamaremos de aquí en adelante API. %\fixme{Fíjate que ésta es una decisión de diseño que aún no has explicado.}
...
\subsubsection{Controlador principal}
 ...
 Es la clase principal %\fxnote{¿es una clase singleton? ¿son todos los controladores singleton? ¿hay varios objetos controladores de los distintos ``tipos'' de controlador?} 
 de la aplicación, común en todas los desarrollos para la plataforma iPhone. Es la encargada de:
 
 \begin{itemize}
	\item La gestión de memoria de la aplicación.
	\item La gestión de los eventos del sistema: describir comportamiento al recibir por ejemplo una llamada telefónica, un correo electrónica\ldots
	\item Será encargado de recoger los eventos del usuario, es decir, de recoger las posiciones de pantalla donde el usuario ha pulsado y lanzar las acciones que el desarrollador haya especificado.
    \end{itemize} 
...
%\begin{afixme}
 ...
 % metódica. Tienes una ``descripción'' y una enumeración de las
  ...
  %quedaría y cuál es el grado de complejidad. -> OK, lo podemos ver en la siguiente reunión.
%\end{afixme}
 
\subsubsection{Controlador de menú principal}
 Esta clase es la encargada de mostrar las opciones principales del programa y  lanzar el controlador adecuado a la elección del usuario %\fxnote{¿puedes lanzar cualquier controlador? ¿podemos recoger esa info en la jerarquía que mencionaba anteriormente?}.
  Los controladores que gestiona son los siguientes:
   \begin{itemize}
	\item Controlador de eventos y mercados.
	\item Controlador de las categorías de mis apuestas.
	\item Controlador de la información de cuenta de Betfair.
	\item Controlador de inicio de sesión para los servicos web de Betfair. Para ello hace uso del servicios \emph{login} de Betfair a través de la clase de comunicación llamada API que explicaremos más adelante.
    \end{itemize} 
    \includegraphics[width=1.2\textwidth]{./images/c_principal.png}
 
\subsubsection{Controlador de tipos de eventos}
 Es el encargado de mostrar al usuario una lista con todos los tipos de eventos deportivos activos de los que dispone Betfair %\fxnote{¿Qué es un evento? Hasta ahora no los has mencionado.} 
 por los que se puede apostar. %\fxnote{Si es futuro no va aquí.}. 
 Una vez seleccionado el evento deseado lanza el controlador de eventos y mercados relacionados con el evento seleccionado. Hace uso del servicio de BetFair \emph{GetActiveEventsTypes}  a través de la clase API del modelo de la aplicación.
 
\subsubsection{Controlador de eventos y mercados}

 Betfair nos proporciona una lista de eventos deportivos donde podemos realizar una apuesta. Dentro de un evento deportivo encontramos la información de mercado relativa al evento, es decir, las cuotas para los distintos protagonistas de los eventos. Esta clase es la encargada de mostrar de forma jerárquica los eventos %\fxnote{subeventos?}
  y mercados %\fxnote{mercados?} 
  relacionados con un eventos seleccionado previamente en el \emph{controlador de eventos}. Gestiona dos tipos de acciones: 
 \begin{itemize}
  \item En caso de ser seleccionado un evento se mostrará los mercados y eventos relaciones con la selección.
  \item En caso de ser seleccionado un mercado se procederá a lanzar el \emph{controlador de información de mercado} . Para ello recaba la información obtenida a través del servicio web \emph{GetEvents} de BetFair.
  \end{itemize}
    
     \includegraphics[width=0.8\textwidth]{./images/c_eventos.png}

\subsubsection{Controlador de información de mercado}

 Esta clase es la responsable de gestionar toda la información relevante al mercado en cuestión. Gestionará el estado del mercado y sus propiedades obteniendo todo lo necesario del portal de Betfair a través de sus servicios web para ello hace uso de las llamadas:
 
  \begin{itemize}
	\item \emph{GetMarket}: obtiene todos los parámetros acerca del mercado.
	\item \emph{GetMarketPrices}: obtiene todos los precios y cuotas actuales del mercado en cuestión.
\end{itemize}
...
\subsubsection{Controlador de las categorías de Mis Apuestas}
 Es la clase encargada de gestionar las apuestas ya realizadas en BetFair por el usuario y que siguen activas. Las clasifica por categorías de mercado. Recoge la información como resultado de las llamadas a los siguientes servicios web de BetFair:
  \begin{itemize}
...
\subsubsection{Controlador de Mis Apuestas}
 Esta clase se encarga de gestionar las apuesta realizadas por el usuario dentro de una categoría determinada. En ella se muestra resumidamente el nombre de la apuesta en cuestión, la cuota apostada y la cuota actualizada en ese momento. 
     ...
     \includegraphics[width=0.8\textwidth]{./images/c_miapuesta.png}


\subsubsection{Controlador de detalles de una apuesta}
 Se encarga de gestionar todos los detalles sobre una apuesta determinada: parámetros, stake, apuesta a favor o en contra relacionadas con la presentada\ldots También incluye información de mercado actual completa referente al mercado de la apuesta y el asesoramiento de trading acerca de la misma.
 
\subsubsection{API: Interfaz a Betfair}
%\fxnote{Habrá que analizar donde colocamos exáctamente esta descripción.}
 Esta clase se encarga de gestionar las comunicaciones con los servidores de BetFair. Envía las peticiones a los servicios web de BetFair y se encarga de realizar la decodificación de la respuesta a dichas peticiones en un formato adecuado para la aplicación. En el apartado \ref{sc:xml} se describe con mas detalle la interfaz.
...
\end{figure}
    ...
    Toda la aplicación gira en torno a este proceso. Es lógico tratarlo así ya que las aplicaciones móviles suelen usar siempre servicios web y por tanto necesitan de conectividad. Esta aplicación no guarda información alguna salvo los datos de acceso al servicio. Es la definición en si de aplicación móvil, la información que necesito disponible este donde este, donde tener información duplicada en el dispositivo no tendría mucho sentido debido a la actualización constante de la información relevantes al mundo de las apuestas, eso sí, tendría sentido quizá dotar en un futuro a la aplicación de una cache con caducidad para minimizar el consumo de batería del dispositivo. Por supuesto estas son todas las ventajas de guardar toda la información en servidores externos a la aplicación. Los contras a cambio suponen un tiempo de retardo entre que se pide la información y se recibe. Afortunadamente hoy el día los terminales están dotados con la tecnología de acceso a la red 3G, lo que supone velocidades de acceso a la red cercana a un ADSL, con lo que el retardo máximo lo podemos estimas en pocos segundos, siendo un tiempo aceptable para este tipo de aplicaciones.  
